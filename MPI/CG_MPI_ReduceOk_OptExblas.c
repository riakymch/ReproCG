...
void fpeSum2( double *in, double *inout, int *len, MPI_Datatype *dptr ) { 
    ...
    double s;
    int size = *len/2;
    fpeSum(in     , inout     , &size, dptr);
    fpeSum(in+size, inout+size, &size, dptr);
}
...
/*
        ...
        // ReproAllReduce -- Begin
*/        
        exblas::exdot_cpu (n_dist, res, res, &fpe[N]);

        // user-defined reduction operations
        MPI_Op Op;
        MPI_Op_create( (MPI_User_function *) fpeSum2, 1, &Op ); 
        if (myId == 0) {
            MPI_Reduce (MPI_IN_PLACE, &fpe[0], 2*N, MPI_DOUBLE, Op, 0, MPI_COMM_WORLD);
        } else {
            MPI_Reduce (&fpe[0], NULL, 2*N, MPI_DOUBLE, Op, 0, MPI_COMM_WORLD);
        }

        if (myId == 0) {
            vAux[0] = exblas::cpu::Round( &fpe[0] );
            vAux[1] = exblas::cpu::Round( &fpe[N] );
        }
