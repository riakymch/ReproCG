...
#pragma omp parallel for
    ...
    // tol = res' * res 
    exblas::exdot_cpu (n_dist, res, res, &fpe_tol[0]);
    ...
    if (myId == 0) {
        MPI_Reduce (MPI_IN_PLACE, &fpe[0], 2*N, MPI_DOUBLE, Op2, 0, MPI_COMM_WORLD);
    } else {
        MPI_Reduce (&fpe[0], NULL, 2*N, MPI_DOUBLE, Op2, 0, MPI_COMM_WORLD);
    }
