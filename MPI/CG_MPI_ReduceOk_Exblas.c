...
#pragma omp parallel for
    ...
    double vAux[2];
    std::vector<int64_t> h_superacc(exblas::BIN_COUNT*2);
...
*/
        ...
        // tol = res' * res
        // ReproAllReduce -- Begin
        exblas::exdot_cpu (n_dist, res, res, &h_superacc[exblas::BIN_COUNT]);
        imin=exblas::IMIN, imax=exblas::IMAX;
        exblas::cpu::Normalize(&h_superacc[exblas::BIN_COUNT], imin, imax);
        if (myId == 0) {
        ...
        if (myId == 0) {
            ...
            vAux[1] = exblas::cpu::Round( &h_superacc[exblas::BIN_COUNT] );
        }
